...
class State(object):
    def __init__(self, logger, totalPlayer, initMoney, bigBlind, button):
        ...
        # bigBlind, every bet should be multiple of smallBlind which is half of bigBlind.
        ...
        # 0, 1, 2, 3 for pre-flop round, flop round, turn round and river round
        self.turnNum = 0
        self.last_raised = bigBlind    # the amount of bet raise last time
        # All players. You can check them to help your decision. The 'cards' field of other player is not visiable for sure.
        self.player = []
    ...
    def set_user_money(self, initMoney):
        for i in range(self.totalPlayer):
            ...
            print('user at pos {} has {}'.format(
                i, self.player[i].money), flush=True)

    def __str__(self):
        repe = 'state: turn= %s currpos = %s, playernum = %s, moneypot = %s, minbet = %s, last_raised = %s \n' \
               % (self.turnNum, self.currpos, self.playernum, self.moneypot, self.minbet, self.last_raised)
